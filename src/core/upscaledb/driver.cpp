...
#include "core/upscaledb/driver.h"
...
#include "core/upscaledb/command.h"              // for Command
#include "core/upscaledb/config.h"               // for Config
#include "core/upscaledb/connection_settings.h"  // for ConnectionSettings
#include "core/upscaledb/database.h"             // for DataBaseInfo
#include "core/upscaledb/db_connection.h"        // for DBConnection
#include "core/upscaledb/server_info.h"          // for ServerInfo, etc
...
Driver::Driver(IConnectionSettingsBaseSPtr settings)
  ...
  COMPILE_ASSERT(DBConnection::connection_t == UPSCALEDB,
                 "DBConnection must be the same type as Driver!");
  CHECK(Type() == UPSCALEDB);
}
...
common::Error Driver::CurrentServerInfo(IServerInfo** info) {
  FastoObjectCommandIPtr cmd = CreateCommandFast(UPSCALEDB_INFO_REQUEST, common::Value::C_INNER);
  LOG_COMMAND(cmd);
  ServerInfo::Stats cm;
...
IServerInfoSPtr Driver::MakeServerInfoFromString(const std::string& val) {
  IServerInfoSPtr res(MakeUpscaleDBServerInfo(val));
  return res;
}

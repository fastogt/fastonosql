...
common::Error CommandTranslator::CreateKeyCommandImpl(const NDbKValue& key, command_buffer_t* cmdstring) const {
  ...
  wr << MAKE_COMMAND_BUFFER(LEVELDB_SET_KEY_COMMAND) << MAKE_COMMAND_BUFFER(" ") << key_str.GetKey() << MAKE_COMMAND_BUFFER(" ") << value_str;
  *cmdstring = wr.str();
  return common::Error();
}
...
common::Error CommandTranslator::LoadKeyCommandImpl(const NKey& key,
  ...
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER(LEVELDB_GET_KEY_COMMAND) << MAKE_COMMAND_BUFFER(" ") << key_str.GetKey();
  *cmdstring = wr.str();
  return common::Error();
}
...
common::Error CommandTranslator::DeleteKeyCommandImpl(const NKey& key, command_buffer_t* cmdstring) const {
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER(LEVELDB_DELETE_KEY_COMMAND) << MAKE_COMMAND_BUFFER(" ") << key_str.GetKey();
  *cmdstring = wr.str();
  return common::Error();
}
...
common::Error CommandTranslator::RenameKeyCommandImpl(const NKey& key,
  ...
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER(LEVELDB_RENAME_KEY_COMMAND) << MAKE_COMMAND_BUFFER(" ") << key_str.GetKey() << MAKE_COMMAND_BUFFER(" ") << new_name;
  *cmdstring = wr.str();
  return common::Error();
}

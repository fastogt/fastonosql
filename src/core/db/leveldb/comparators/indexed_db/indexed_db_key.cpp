...
IndexedDBKey::IndexedDBKey() : type_(kWebIDBKeyTypeNull), size_estimate_(kOverheadSize) {}
IndexedDBKey::IndexedDBKey(WebIDBKeyType type) : type_(type), size_estimate_(kOverheadSize) {
  DCHECK(type == kWebIDBKeyTypeNull || type == kWebIDBKeyTypeInvalid);
}
IndexedDBKey::IndexedDBKey(double number, WebIDBKeyType type)
    : type_(type), number_(number), size_estimate_(kOverheadSize + sizeof(number)) {
...
IndexedDBKey::IndexedDBKey(const common::string16& string)
    : type_(kWebIDBKeyTypeString),
      string_(string),
      size_estimate_(kOverheadSize + (string.length() * sizeof(common::string16::value_type))) {}

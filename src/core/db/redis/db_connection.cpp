...
common::Error DBConnection::SetEx(const NDbKValue& key, ttl_t ttl) {
  ...
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER("SETEX ") << key_str.GetKeyData() << MAKE_COMMAND_BUFFER(" ") << ttl
...
common::Error DBConnection::Lrange(const NKey& key, int start, int stop, NDbKValue* loaded_key) {
  ...
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER("LRANGE ") << key_str.GetKeyData() << MAKE_COMMAND_BUFFER(" ") << start
...
common::Error DBConnection::Zrange(const NKey& key, int start, int stop, bool withscores, NDbKValue* loaded_key) {
  ...
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER("ZRANGE ") << key_str.GetKeyData() << MAKE_COMMAND_BUFFER(" ") << start
     << MAKE_COMMAND_BUFFER(" ") << stop;
  if (withscores) {
...
common::Error DBConnection::DecrBy(const NKey& key, int dec, long long* decr) {
  ...
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER("DECRBY ") << key_str.GetKeyData() << MAKE_COMMAND_BUFFER(" ") << dec;
...
common::Error DBConnection::IncrBy(const NKey& key, int inc, long long* incr) {
  ...
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER("INCRBY ") << key_str.GetKeyData() << MAKE_COMMAND_BUFFER(" ") << inc;
...
common::Error DBConnection::IncrByFloat(const NKey& key, double inc, std::string* str_incr) {
  ...
  key_t key_str = key.GetKey();
  command_buffer_writer_t wr;
  wr << MAKE_COMMAND_BUFFER("INCRBYFLOAT ") << key_str.GetKeyData() << MAKE_COMMAND_BUFFER(" ") << inc;

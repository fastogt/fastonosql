...
common::Error CommandTranslator::ChangeKeyTTLCommandImpl(const NKey& key, ttl_t ttl, std::string* cmdstring) const {
  ...
  std::string errorMsg =
      common::MemSPrintf("Sorry, but now " PROJECT_NAME_TITLE " not supported change ttl command for RocksDB.");
...
common::Error CommandTranslator::LoadKeyTTLCommandImpl(const NKey& key, std::string* cmdstring) const {
  ...
  std::string errorMsg =
      common::MemSPrintf("Sorry, but now " PROJECT_NAME_TITLE " not supported get ttl command for RocksDB.");
  return common::make_error_value(errorMsg, common::ErrorValue::E_ERROR);
}
...
common::Error CommandTranslator::PublishCommandImpl(const NDbPSChannel& channel,
  ...
  std::string errorMsg =
      common::MemSPrintf("Sorry, but now " PROJECT_NAME_TITLE " not supported publish command for RocksDB.");
  return common::make_error_value(errorMsg, common::ErrorValue::E_ERROR);
}
...
common::Error CommandTranslator::SubscribeCommandImpl(const NDbPSChannel& channel, std::string* cmdstring) const {
  ...
  std::string errorMsg =
      common::MemSPrintf("Sorry, but now " PROJECT_NAME_TITLE " not supported subscribe command for RocksDB.");
  return common::make_error_value(errorMsg, common::ErrorValue::E_ERROR);
}

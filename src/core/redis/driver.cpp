...
void Driver::handleClearDatabaseEvent(events::ClearDatabaseRequestEvent* ev) {
  ...
  notifyProgress(sender, 50);
  FastoObjectCommandIPtr cmd = CreateCommand<Command>(root, REDIS_FLUSHDB,
...
void Driver::handleSetDefaultDatabaseEvent(events::SetDefaultDatabaseRequestEvent* ev) {
  ...
  notifyProgress(sender, 50);
  FastoObjectCommandIPtr cmd = CreateCommand<Command>(root, setDefCommand,
                                                        common::Value::C_INNER);
  common::Error er = execute(cmd);
  if (er && er->isError()) {
    res.setErrorInfo(er);
...
void Driver::handleLoadServerPropertyEvent(events::ServerPropertyInfoRequestEvent* ev) {
  ...
  notifyProgress(sender, 50);
  FastoObjectCommandIPtr cmd = CreateCommand<Command>(root, REDIS_GET_PROPERTY_SERVER,
                                                        common::Value::C_INNER);
  common::Error er = execute(cmd);
  if (er && er->isError()) {
    res.setErrorInfo(er);
...
void Driver::handleServerPropertyChangeEvent(events::ChangeServerPropertyInfoRequestEvent* ev) {
  ...
  FastoObjectCommandIPtr cmd = CreateCommand<Command>(root, changeRequest,
                                                        common::Value::C_INNER);
  common::Error er = execute(cmd);
  if (er && er->isError()) {
    res.setErrorInfo(er);

...
static const std::vector<CommandInfo> redisCommands = {
  ...
  CommandInfo("CLIENT KILL", "[ip:port] [ID client-id] [TYPE normal|master|slave|pubsub] [ADDR ip:port] [SKIPME yes/no]",
  ...
  CommandInfo("CLIENT REPLY", "ON|OFF|SKIP",
              "Instruct the server whether to reply to commands", PROJECT_VERSION_GENERATE(3,2,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  ...
  CommandInfo("CLUSTER ADDSLOTS", "<slot> [slot ...]",
              "Assign new hash slots to receiving node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, INFINITE_COMMAND_ARGS),
  CommandInfo("CLUSTER COUNT-FAILURE-REPORTS", "<node-id>",
              "Return the number of failure reports active for a given node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  CommandInfo("CLUSTER COUNTKEYSINSLOT", "<slot>",
              "Return the number of local keys in the specified hash slot", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  CommandInfo("CLUSTER DELSLOTS", "<slot> [slot ...]",
              "Set hash slots as unbound in receiving node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, INFINITE_COMMAND_ARGS),
  CommandInfo("CLUSTER FAILOVER", "[FORCE|TAKEOVER]",
              "Forces a slave to perform a manual failover of its master.", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 1),
  CommandInfo("CLUSTER FORGET", "<node-id>",
              "Remove a node from the nodes table", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  CommandInfo("CLUSTER GETKEYSINSLOT", "<slot> <count>",
              "Return local key names in the specified hash slot", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 2, 0),
  CommandInfo("CLUSTER INFO", "-",
              "Provides info about Redis Cluster node state", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 0),
  CommandInfo("CLUSTER KEYSLOT", "<key>",
              "Returns the hash slot of the specified key", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  CommandInfo("CLUSTER MEET", "<ip> <port>",
              "Force a node cluster to handshake with another node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 2, 0),
  CommandInfo("CLUSTER NODES", "-",
              "Get Cluster config for the node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 0),
  CommandInfo("CLUSTER REPLICATE", "<node-id>",
              "Reconfigure a node as a slave of the specified master node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  CommandInfo("CLUSTER RESET", "[HARD|SOFT]",
              "Reset a Redis Cluster node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 1),
  CommandInfo("CLUSTER SAVECONFIG", "-",
              "Forces the node to save cluster state on disk", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 0),
  CommandInfo("CLUSTER SET-CONFIG-EPOCH", "<config-epoch>",
              "Set the configuration epoch in a new node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  CommandInfo("CLUSTER SETSLOT", "<slot> IMPORTING|MIGRATING|STABLE|NODE [node-id]",
              "Bind a hash slot to a specific node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 2, 1),
  CommandInfo("CLUSTER SLAVES", "<node-id>",
              "List slave nodes of the specified master node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  CommandInfo("CLUSTER SLOTS", "-",
              "Get array of Cluster slot to node mappings", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 0),

  CommandInfo("COMMAND", "-",
              "Get array of Redis command details", PROJECT_VERSION_GENERATE(2,8,13), UNDEFINED_EXAMPLE_STR, 0, 0),
  CommandInfo("COMMAND COUNT", "-",
              "Get total number of Redis commands", PROJECT_VERSION_GENERATE(2,8,13), UNDEFINED_EXAMPLE_STR, 0, 0),
  CommandInfo("COMMAND GETKEYS", "-",
              "Extract keys given a full Redis command", PROJECT_VERSION_GENERATE(2,8,13), UNDEFINED_EXAMPLE_STR, 0, 0),
  CommandInfo("COMMAND INFO", "command-name [command-name ...]",
              "Get array of specific Redis command details", PROJECT_VERSION_GENERATE(2,8,13), UNDEFINED_EXAMPLE_STR, 1, 0),
  ...
  CommandInfo("EXISTS",
              "key [key ...]", "Determine if a key exists", PROJECT_VERSION_GENERATE(1,0,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  ...
  CommandInfo("GEOADD", "key longitude latitude member [longitude latitude member ...",
              "Add one or more geospatial items in the geospatial index represented using a sorted set", UNDEFINED_SINCE, UNDEFINED_EXAMPLE_STR, 4, 0),
  CommandInfo("GEODIST", "key member1 member2 [unit]",
              "Returns the distance between two members of a geospatial index", UNDEFINED_SINCE, UNDEFINED_EXAMPLE_STR, 3, 0),
  CommandInfo("GEOHASH", "key member [member ...]",
              "Returns members of a geospatial index as standard geohash strings", UNDEFINED_SINCE, UNDEFINED_EXAMPLE_STR, 2, 0),
  CommandInfo("GEOPOS", "key member [member ...]",
              "Returns longitude and latitude of members of a geospatial index", UNDEFINED_SINCE, UNDEFINED_EXAMPLE_STR, 2, 0),

  CommandInfo("GEORADIUS", "key longitude latitude radius m|km|ft|mi [WITHCOORD] [WITHDIST] [WITHHASH] [COUNT count] [ASC|DESC]",
              "Query a sorted set representing a geospatial index to fetch members matching a given maximum distance from a point", UNDEFINED_SINCE, UNDEFINED_EXAMPLE_STR, 5, 0),
  CommandInfo("GEORADIUSBYMEMBER", "key member radius m|km|ft|mi [WITHCOORD] [WITHDIST] [WITHHASH] [COUNT count] [ASC|DESC]",
              "Query a sorted set representing a geospatial index to fetch members matching a given maximum distance from a member", UNDEFINED_SINCE, UNDEFINED_EXAMPLE_STR, 4, 0),
  CommandInfo("GET", "<key>",
  ...
  CommandInfo("HSTRLEN", "<key> <field>",
              "Get the length of the value of a hash field", PROJECT_VERSION_GENERATE(3,2,0), UNDEFINED_EXAMPLE_STR, 2, 0),
  ...
  CommandInfo("MIGRATE", "<host> <port> <|""> <destination-db> <timeout> [COPY] [REPLACE]",
  ...
  CommandInfo("PING", "[message]",
              "Ping the server", PROJECT_VERSION_GENERATE(1,0,0), UNDEFINED_EXAMPLE_STR, 0, 1),
  ...
  CommandInfo("READONLY", "-",
              "Enables read queries for a connection to a cluster slave node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 0),
  CommandInfo("READWRITE", "-",
              "Disables read queries for a connection to a cluster slave node", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 0, 0),
  ...
  CommandInfo("RESTORE", "<key> <ttl> <serialized-value> [REPLACE]",
  ...
  CommandInfo("ROLE", "-",
              "Return the role of the instance in the context of replication", PROJECT_VERSION_GENERATE(2,8,12), UNDEFINED_EXAMPLE_STR, 1, 0),
  ...
  CommandInfo("RPOPLPUSH", "<source> <destination>",
              "Remove the last element in a list, prepend it to another list and return it", PROJECT_VERSION_GENERATE(1,2,0), UNDEFINED_EXAMPLE_STR, 2, 0),
  ...
  CommandInfo("SCRIPT DEBUG", "<YES|SYNC|NO>",
              "Set the debug mode for executed scripts.", PROJECT_VERSION_GENERATE(3,2,0), UNDEFINED_EXAMPLE_STR, 1, 0),
  ...
  CommandInfo("WAIT", "numslaves timeout",
              "Wait for the synchronous replication of all the write commands sent in the context of the current connection", PROJECT_VERSION_GENERATE(3,0,0), UNDEFINED_EXAMPLE_STR, 2, 0),
  ...
  CommandInfo("ZADD", "<key> [NX|XX] [CH] [INCR] <score> <member> [score member ...]",
  ...
  CommandInfo("ZREVRANGEBYLEX", "<key> <max> <min> [LIMIT offset count]",
              "Return a range of members in a sorted set, by lexicographical range, ordered from higher to lower strings.", PROJECT_VERSION_GENERATE(2,8,9), UNDEFINED_EXAMPLE_STR, 3, 1),
